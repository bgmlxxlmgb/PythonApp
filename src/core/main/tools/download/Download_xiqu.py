...
list_arr = []
for i in range(1,341): #1214
    try:
        url='http://k6.csnmdcjnx.com/pw/thread.php?fid=83&page='+str(i)
        content = core(url)
        soup = BeautifulSoup(content, "html.parser")
        if soup.text.find("金瓶梅")>0 or soup.text.find("金瓶")>10 or soup.text.find("满清")>0:
            print(str(1)+"|"+url)
    except:
        print(str(i)+"-")